...
const MOOD_THEMES = {
  'Dream State': {
    ...
    // EE tint pulled back to a pale-throughout palette — the previous
    // purple-dominant version stuck out against pink-cream parts of
    // the Dream State sky, the previous pink-dominant version stuck
    // out against lavender parts. Pale-throughout sits in the same
    // value range as the regular pastel cloud field regardless of
    // where the EE happens to drift across the sky gradient.
    ...
    fluffOpacity: 0.3,        // pastels bloom out at default 0.6; halved
  },
  'Golden Hour': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #b88474 0%, #d49c80 25%, #ecb088 50%, #f8c490 75%, #ffd880 100%)',
  ...
  'Moonlight': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #d4dceb 0%, #b0bcd4 30%, #8898bc 60%, #5e7098 85%, #45567c 100%)',
  ...
  'Blush Hour': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #ffe8e6 0%, #fcd0d2 25%, #f4bcc8 55%, #ec98b4 80%, #dc80a4 100%)',
  ...
  'Coastal Morning': {
    ...
    // Underbelly glow — Coastal Morning is bottom-lit (sun rising at
    // horizon). Bumped opacity 0.20 → 0.34 + dropped sepia 0.18 →
    // 0.08 so the glow paints brighter white-cream highlights on the
    // EE silhouette instead of just a warm tint. Mask widened to
    // start higher up the silhouette so more of the body catches it.
    eggGlow: {
      opacity: 0.34,
  ...
  'Greenhouse': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #e8e4c4 0%, #dcd8b4 25%, #ccd0a0 50%, #b8c490 75%, #a4b47c 100%)',
  ...
  'Neon Nights': {
    ...
    // Neon Nights EE — was reading near-white against the lavender-
    // purple realistic cloud field (top stop #d8c8dc was too pale).
    // Pushed every stop into the saturated lavender-purple zone the
    // realistic Neon clouds actually occupy so the silhouette belongs
    // in the field, not stamped onto it.
    eggTintGradient: 'linear-gradient(172deg, #d4c0e0 0%, #b8a0d0 25%, #9880c0 50%, #7660a0 75%, #54447c 100%)',
    ...
    // Neon's stacked colored drop-shadows already paint heavy color spill
    // into the sky — the fluff halo on top doubles up. Almost off here.
    fluffOpacity: 0.15,
  },
  'Vivid Sunset': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #5c3878 0%, #c45888 25%, #e86090 50%, #f48894 75%, #fcb898 100%)',
  ...
  'Bright Day': {
    ...
    eggTintGradient: 'linear-gradient(180deg, #fafdff 0%, #e0ecf8 25%, #b0ccea 55%, #78a4d4 80%, #4880c8 100%)',
  ...
  return (
    <div style={{
      ...
      {initial.map((s, idx) => {
        ...
        // Themed rendering: tint + shade + glow. EE-slot puffs use the
        // lighter `eggTintGradient` (v4 handoff) so shape clouds read as
        // soft puffs that share the mood's palette, not crisp tinted
        // cutouts.
        const isEeSlot = forceEasterEggs && eeSlotSet.has(idx)
        const tintGradient = isEeSlot && theme.eggTintGradient ? theme.eggTintGradient : theme.tintGradient
        return (
          <div
            key={idx}
            ref={el => { wrapRefs.current[idx] = el }}
          ...
          >
            ...
            {/* No aura on EE clouds — every halo variant we've tried
                (screen-blended white PNG, tinted blurred silhouette)
                reads as a visible ring outside the cloud body. The PNG's
                own fluffy alpha edge + tint + shade give the silhouette
                enough cloud quality; the EE shape stands out because the
                silhouette IS the easter egg, not because of a glow. */}
