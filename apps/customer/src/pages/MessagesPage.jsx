...
// Customer-facing inbox. Mounts via `?messages=1`. Lists every conversation
// the user is part of (with any seller, with or without an order), grouped
// by (partner, order). Click a conversation to expand the thread inline.
...
export default function MessagesPage({ onBack }) {
    ...
    async function load() {
      ...
      const { data } = await supabase
        ...
        .select(`
          ...
          from_profile:profiles!messages_from_user_id_fkey(display_name, avatar_url),
          to_profile:profiles!messages_to_user_id_fkey(display_name, avatar_url),
  ...
  const conversations = useMemo(() => {
    ...
    for (const m of rows) {
      ...
      const partnerProf = m.from_user_id === user.id ? m.to_profile : m.from_profile
      const partnerName = partnerProf?.display_name || 'Seller'
      const partnerAvatar = partnerProf?.avatar_url || null
  ...
  if (!user) {
    return (
      <div style={s.page}>
        <div style={s.empty}>
          <h2 style={s.emptyTitle}>Sign in to see your messages</h2>
          <p style={s.dim}>Conversations with sellers live here.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={s.page}>
      <header style={s.header}>
        ...
        <h1 style={s.title}>Your Messages</h1>
      </header>
      ...
      {loading ? (
        ...
        <div style={s.empty}>
          ...
          <p style={s.dim}>Reach out from a seller's profile or your order detail to start a thread.</p>
        </div>
      ) : (
        <div style={s.list}>
          {conversations.map(conv => (
            <div key={conv.key} style={s.card}>
              ...
              >
                <Avatar url={conv.partnerAvatar} name={conv.partnerName} t={t} />
...
function Avatar({ url, name, t }) {
  const initial = (name ?? '?')[0].toUpperCase()
  if (url) {
    return <img src={url} alt="" style={{ width: 38, height: 38, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
  }
  return (
    <div style={{ width: 38, height: 38, borderRadius: '50%', background: t.accent, color: t.accentText, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, flexShrink: 0 }}>
      {initial}
    </div>
  )
}
...
function makeStyles(t) {
  return {
    ...
    header:      { display: 'flex', alignItems: 'center', gap: 14, maxWidth: 760, margin: '0 auto 24px' },
    backBtn:     { background: 'transparent', border: 'none', color: t.accent, fontSize: 14, cursor: 'pointer', padding: '4px 8px', fontWeight: 500 },
    title:       { fontSize: 28, fontWeight: 700, color: t.text, margin: 0 },
    dim:         { fontSize: 13, color: t.textSoft, margin: 0 },
    empty:       { padding: 40, background: t.surface, border: `1px dashed ${t.surfaceBorder}`, borderRadius: 14, textAlign: 'center', maxWidth: 520, margin: '40px auto' },
    ...
    card:        { background: t.surface, border: `1px solid ${t.surfaceBorder}`, borderRadius: 14, overflow: 'hidden' },
