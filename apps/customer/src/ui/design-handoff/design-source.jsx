...
function BuilderApp({ device, mode, mood, theme, onMood }) {
  const geom = geomFor(mode);
  const [items, setItems] = useState(SEED);
  const [sel, setSel] = useState(null);
  const [tool, setTool] = useState(mode === 'mobile' ? null : 'place');
  const [detail, setDetail] = useState(null);       // { key, sizeIndex, swatchIndex }
  ...
  // reset tool default when switching layout
  useEffect(() => { setTool(mode === 'mobile' ? null : 'place'); setDetail(null); setCartOpen(false); }, [mode]);
